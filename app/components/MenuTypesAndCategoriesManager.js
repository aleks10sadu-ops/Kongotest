...
export default function MenuTypesAndCategoriesManager({ isOpen, onClose }) {
  const [menuTypes, setMenuTypes] = useState([]);
  const [categories, setCategories] = useState({}); // { menuTypeId: [categories] }
  ...
  const [newMenuType, setNewMenuType] = useState({ name: '', slug: '', description: '' });
  const [newCategory, setNewCategory] = useState({ name: '', note: '', menu_type_id: '' });
  ...
  const handleSaveMenuType = async (menuType) => {
    try {
      const supabase = createSupabaseBrowserClient();
      if (!supabase) return;
  ...
  const handleDeleteMenuType = async (menuTypeId) => {
  ...
  const toggleMenuType = (menuTypeId) => {
