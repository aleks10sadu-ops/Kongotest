 ...
 * @param {Object} props
 * @param {Object} props.item - данные блюда
 * @param {Function} props.onAddToCart - добавление в корзину
 * @param {Function} props.onItemClick - клик по карточке (открытие модального окна)
 * @param {Array} props.cartItems - текущие элементы корзины
 * @param {boolean} props.isAdmin - является ли пользователь админом
 * @param {boolean} props.editMode - режим редактирования
 * @param {Array} props.allCategories - все категории для выбора
 * @param {boolean} props.priority - приоритетная загрузка изображения (для первых элементов)
 */
export default function MenuItem({
  ...
  const [adminWeight, setAdminWeight] = useState(item.weight || '');
  const [adminImageUrl, setAdminImageUrl] = useState(item.image || '');
  const [adminCategoryId, setAdminCategoryId] = useState(item.categoryId || '');
...
function AdminEditFields({ adminName, setAdminName, adminPrice, setAdminPrice, adminWeight, setAdminWeight }) {
  return (
...
function VariantsList({ item, variants, getVariantQuantity, handleAdd, handleRemove }) {
  return (
...
function QuantityControls({ quantity, onAdd, onRemove, compact = false }) {
...
function AdminControls({ item, adminImageUrl, setAdminImageUrl, adminCategoryId, setAdminCategoryId, allCategories, adminError, adminSaving, onSave, onDelete }) {
  return (
